...
namespace TgcViewer.Utils.TgcGeometry
{
    ...
    /// </summary>
    public class TgcCustomEditableLand : IRenderObject, ITransformObject
    {
        int EDITABLE_GRID_SIZE;
        int LATERAL_SIZE;
        ...
        /// Crea el terreno
        /// </summary>
        public TgcCustomEditableLand(int grid_size, int lateral_size)
        {
            EDITABLE_GRID_SIZE = grid_size;
            LATERAL_SIZE = lateral_size;
            Device d3dDevice = GuiController.Instance.D3dDevice;

            //16 caras, 32 triangulos, 96 vertices
            vertices = new CustomVertex.PositionTextured[LATERAL_SIZE * LATERAL_SIZE * 2 * 3];
            ...
            editableVertices = new EditableVertex[EDITABLE_GRID_SIZE * EDITABLE_GRID_SIZE];
            float uvStep = 1f / (float)LATERAL_SIZE;
            for (int i = 0; i < EDITABLE_GRID_SIZE; i++)
            {
                for (int j = 0; j < EDITABLE_GRID_SIZE; j++)
                {
                    EditableVertex v = new EditableVertex();
                    v.Pos = new Vector3(j * PATCH_SIZE, 0, i * PATCH_SIZE);
                    v.UV = new Vector2(j * uvStep, i * uvStep);
                    editableVertices[i * EDITABLE_GRID_SIZE + j] = v;
                }
            }
        ...
        public void updateValues()
        {
            ...
            {
                for (int j = 0; j < LATERAL_SIZE; j++)
                {
                    //Los 4 vertices que forman la cara
                    EditableVertex tl = editableVertices[i * EDITABLE_GRID_SIZE + j];
                    EditableVertex tr = editableVertices[i * EDITABLE_GRID_SIZE + j + 1];
                    EditableVertex bl = editableVertices[(i + 1) * EDITABLE_GRID_SIZE + j];
                    EditableVertex br = editableVertices[(i + 1) * EDITABLE_GRID_SIZE + j + 1];
        ...
        public void render()
        {
            ...
            effect.Begin(0);
            effect.BeginPass(0);
            d3dDevice.DrawPrimitives(PrimitiveType.TriangleList, 0, LATERAL_SIZE * LATERAL_SIZE * 2);
            effect.EndPass();
            effect.End();
        ...
        public void updateBoundingBox()
        {
            ...
            boundingBox.setExtremes(new Vector3(0, minY, 0), new Vector3(PATCH_SIZE * LATERAL_SIZE, maxY, PATCH_SIZE * LATERAL_SIZE));
